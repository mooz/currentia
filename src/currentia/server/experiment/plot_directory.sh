...
plot \
 "${data}" ind 1:1 usi 1:2 ti "${lock_label}" w points 7 7,\
 "${data}" ind 2:2 usi 1:2 ti "${snapshot_label}" w points 7 9
...
for i in $(seq 1 5); do
    ...
    ./format_benchmark.rb ${DIR}/update_vs_stream/ update_throughput query_throughput > /tmp/throughput_throughput_${i}.txt
done
...
./calculate_average.rb /tmp/throughput_throughput_1.txt /tmp/throughput_throughput_2.txt /tmp/throughput_throughput_3.txt /tmp/throughput_throughput_4.txt /tmp/throughput_throughput_5.txt > /tmp/throughput_throughput.txt
...
plot_scatter /tmp/throughput_throughput.txt "Update Throughput (update/sec)" "Query Throughput (query/sec)" > ./throughput_throughput.svg
